...
// Replaces: Text2D.vert + Text2D.geom + Text.frag
// Used by: Text2D
//
// Strategy: Each character is an instance with a world-space anchor position (Vec2F),
// a character bounding box (Vec4S), and a texture offset (int). The vertex shader
// transforms the anchor to clip space, converts to pixel coordinates, computes the
// character quad corners in pixel space, then converts back to clip space.
// The fragment shader samples the font texture.
...
fn vs_main(@builtin(vertex_index) vid: u32, input: VertexInput) -> VertexOutput {
    ...
    xyref = xyref + vec2<f32>(0.01, 0.01);  // delta to avoid truncation errors
