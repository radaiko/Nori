// ────── ╔╗
// ╔═╦╦═╦╦╬╣ Program.cs
// ║║║║╬║╔╣║ Standalone Nori server — demo for networked architecture
// ╚╩═╩═╩╝╚╝ ──────────────────────────────────────────────────────────
using Nori;

string url = args.Length > 0 ? args[0] : "http://0.0.0.0:5100/";
Console.WriteLine ($"Nori Standalone Server");
...
server.OnClientConnected += session =>
   Console.WriteLine ($"[{DateTime.Now:HH:mm:ss}] Client connected: {session.Id}");
server.OnClientDisconnected += session =>
   Console.WriteLine ($"[{DateTime.Now:HH:mm:ss}] Client disconnected: {session.Id}");
