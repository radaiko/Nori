...
class HullNode : VNode {
   ...
   public override void Draw () {
      List<Vec2F> pts = mPts.Select (a => (Vec2F)a).ToList ();
      Lux.Points (pts.AsSpan ());
      List<Point2> hull = ConvexHull.Compute (mPts);
      Lux.LineLoop (hull);
   }

   /// <summary>Add a point to the hull set and redraw</summary>
   ...
   /// <summary>Add random points and redraw</summary>
   ...
   void AddRandom (int c) {
      ...
      using BlockTimer bt = new ($"Hull of {mPts.Count} pts");
      List<Point2> hull = ConvexHull.Compute (mPts);
   }
