...
class MinSphereScene : Scene3 {
   ...
   void Build (Point3[] pts) {
      ...
      sw.Restart ();
      MinSphere s2 = MinSphere.FromQuickApprox (pts);
      sw.Stop ();
      ...
      sw.Stop ();
      nodes.Add (new MESBoxVN (obb));
      Bound3 aabb = new (pts);
      double vol = obb.Volume;
      double abvol = aabb.Width * aabb.Height * aabb.Depth;
      double svol = 4 / 3 * Lib.PI * Math.Pow (s.Radius, 3);
   ...
   static readonly (Color4 Clr, float Size)[] Styles = [(Color4.Green, 8), (Color4.White, 3), (Color4.Red, 8)];
   ...
   class AxesVN : VNode {
      ...
      static readonly Vec3F Org = new ();
   }

   // Draw OBB.
   class MESBoxVN (OBB box) : VNode {
      ...
      public override void Draw () {
         if (Pts.Length == 0) {
            List<Vec3F> corners = [];
            (float ex, float ey, float ez) = ((float)Box.Extent.X, (float)Box.Extent.Y, (float)Box.Extent.Z);
